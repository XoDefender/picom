...
image_handle backend_compat_bind_pixmap(struct backend_base *base, xcb_pixmap_t pixmap, struct xvisual_info visual) {
	auto g = XCB_AWAIT(xcb_get_geometry, base->c, pixmap);
	if (!g) {
...
backend_compat_create_shadow_context(struct backend_base *base, double radius) {
	...
	if (radius > 0) {
		...
		ctx->blur_context = base->ops->create_blur_context(base, BLUR_METHOD_GAUSSIAN, BACKEND_IMAGE_FORMAT_MASK, &args);
		if (!ctx->blur_context) {
...
bool backend_compat_init(struct backend_compat_base *compat, struct session *ps) {
	...
	compat->format = BACKEND_IMAGE_FORMAT_PIXMAP;
