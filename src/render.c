...
static inline bool paint_bind_tex(session_t *ps, paint_t *ppaint, int wid, int hei,
	...
	if (!visual) {
		...
		if (!ppaint->fbcfg.cfg) {
			glx_find_fbconfig(ps->dpy, ps->scr, m, &ppaint->fbcfg);
		}
		if (!ppaint->fbcfg.cfg) {
...
static bool win_build_shadow(session_t *ps, struct managed_win *w, double opacity) {
	...
	xcb_render_picture_t shadow_color_picture = win_get_shadow_picture(ps, w);

	shadow_image = make_shadow(ps->c, (conv *)shadow_context, opacity, width, height);
	if (!shadow_image) {
	...
	xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_SRC, shadow_color_picture,
...
win_paint_shadow(session_t *ps, struct managed_win *w, region_t *reg_paint) {
	...
	double shadow_opacity = win_get_shadow_opacity(ps, w);
...
// Generates another Picture for shadows if the color is modified by user
bool init_shadow_picts(session_t *ps)
{
	if (ps->o.shadow_red == 0 && ps->o.shadow_green == 0 && ps->o.shadow_blue == 0) {
		ps->cshadow_picture = ps->black_picture;
	} else {
		ps->cshadow_picture = solid_picture(ps->c, ps->root, true, 1
											, ps->o.shadow_red
											, ps->o.shadow_green
											, ps->o.shadow_blue);
	...
	if (ps->o.shadow_red_active == 0 &&
		ps->o.shadow_green_active == 0 &&
		ps->o.shadow_blue_actives == 0) {
			ps->cshadow_picture_active = ps->black_picture;
	}
	else {
		ps->cshadow_picture_active = solid_picture(ps->c, ps->root, true, 1
												   , ps->o.shadow_red_active
												   , ps->o.shadow_green_active
												   , ps->o.shadow_blue_actives);

		if (ps->cshadow_picture_active == XCB_NONE) {
			log_error("Failed to create shadow picture for the active window.");
			return false;
		}
	}
...
// Free shadow pictures
void deinit_shadow_picts(session_t *ps)
{
	...
	// TODO:Kirill
	if (ps->cshadow_picture_active == ps->black_picture)
		ps->cshadow_picture_active = XCB_NONE;
	else
		free_picture(ps->c, &ps->cshadow_picture_active);
}
...
bool init_render(session_t *ps) {
	...
	if(!init_shadow_picts(ps)) return false;
