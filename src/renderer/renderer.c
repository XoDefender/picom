...
renderer_init(struct renderer *renderer, struct backend_base *backend,
              double shadow_radius, struct color shadow_color) 
			  {

	auto has_high_precision = backend->ops->v2.is_format_supported(backend, BACKEND_IMAGE_FORMAT_PIXMAP_HIGH);
	renderer->format = BACKEND_IMAGE_FORMAT_PIXMAP;
	...
	if (shadow_radius > 0) {
		...
		renderer->shadow_pixel =
		    solid_picture(backend->c, backend->root, true, shadow_color.alpha, shadow_color.red,
...
struct renderer *renderer_new(struct backend_base *backend, double shadow_radius,
	...
	if (!renderer_init(renderer, backend, shadow_radius, shadow_color)) {
...
bool renderer_render(struct renderer *r, struct backend_base *backend,
	...
	command_builder_build(cb, layout, force_blend, blur_frame, inactive_dim_fixed,
	                      max_brightness, inactive_dim, shadow_exclude, nscreens, screens,
	                      wintype_options);
